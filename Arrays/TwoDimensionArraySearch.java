...
public class TwoDimensionArraySearch {
    ...
    // Constructor
    public TwoDimensionArraySearch(int numberOfRows, int numberOfColumns) {
        ...
        for (int row = 0; row < arr.length; row++) {   // Time Complexity O(m)
            for (int col = 0; col < arr[0].length; col++) {   // Time Complexity O(n)
                if (arr[row][col] == value) {
                    System.out.println("The Value is found at the row " + row + " column " + col);   // Time Complexity O(1)
                    return;
                }
            }
        }
        System.out.println("Value not found in Array");   // Time Complexity O(1)
    }

    public static void main(String[] args) {
        TwoDimensionArraySearch obj2DArray = new TwoDimensionArraySearch(3, 3);
        ...
        obj2DArray.searchValueInArray(80);
